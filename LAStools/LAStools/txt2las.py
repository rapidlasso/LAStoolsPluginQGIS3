...
class txt2las(LAStoolsAlgorithm):
    ...
    def initAlgorithm(self, config):
        ...
        self.addParameter(QgsProcessingParameterNumber(txt2las.SKIP,
                                          self.tr("skip the first n lines"), 0, None, 0))
        self.addParameter(QgsProcessingParameterNumber(txt2las.SCALE_FACTOR_XY,
                                          self.tr("resolution of x and y coordinate"), 0, None, 0.01))
        self.addParameter(QgsProcessingParameterNumber(txt2las.SCALE_FACTOR_Z,
                                          self.tr("resolution of z coordinate"), 0, None, 0.01))
        self.addParameter(QgsProcessingParameterEnum(txt2las.PROJECTION,
                                             self.tr("projection"), txt2las.PROJECTIONS, 0))
        self.addParameter(QgsProcessingParameterEnum(txt2las.UTM,
                                             self.tr("utm zone"), txt2las.UTM_ZONES, 0))
        self.addParameter(QgsProcessingParameterEnum(txt2las.SP,
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        commands.append("-i")
        commands.append(self.parameterAsInt(parameters, txt2las.INPUT))
        parse_string = self.parameterAsInt(parameters, txt2las.PARSE)
        if parse_string != "xyz":
            commands.append("-parse")
            commands.append(parse_string)
        skip = self.parameterAsInt(parameters, txt2las.SKIP)
        if parse_string != 0:
            commands.append("-skip")
            commands.append(unicode(skip))
        scale_factor_xy = self.parameterAsInt(parameters, txt2las.SCALE_FACTOR_XY)
        scale_factor_z = self.parameterAsInt(parameters, txt2las.SCALE_FACTOR_Z)
        ...
        projection = self.parameterAsInt(parameters, txt2las.PROJECTION)
        if projection != 0:
            if projection == 1:
                utm_zone = self.parameterAsInt(parameters, txt2las.UTM)
                if utm_zone != 0:
                ...
                sp_code = self.parameterAsInt(parameters, txt2las.SP)
                if sp_code != 0:
                    commands.append("-" + txt2las.PROJECTIONS[projection])
